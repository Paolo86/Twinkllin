...
$ps = $con->prepare("select validation,active, Activation_expire from " . userTable() ." where email=?;");
$ps->bind_param("s",$email);
$result = $ps->execute();

    $ps->bind_result($codeIn,$isActive, $expire);
...
else if($isActive == 1 && $codeIn == $code)
{
	echo '
	<link rel="stylesheet" href="../css/resetstyle.css" />
	<link rel="stylesheet" type="text/css" href="../css/main.css" />
...
else if($codeIn != $code)
{
	...
	<div id="newPswMessage" style="margin: auto; width: 50%; text-align: center">
						...
						<p class="formLabel" style="color: #c0c0c0;  ">This link is expired.</p>
